...
lval *lval_lambda(lval *formals, lval *body) {
  ...
  // Set built in to Null. This is to distinguish the two types of functions
  v->builtin = NULL;
...
lval *lval_call(lenv *e, lval *f, lval *a) {
  ...
  // if all formals have been boud evaluate
  if (f->formals->count == 0) {
    // Set the parent env
    f->env->par = e;
...
lval *builtin_var(lenv *e, lval *a, char *func) {
  ...
  for (int i = 0; i < syms->count; i++) {
    /* If 'def' define in global env, if 'put define in local env' */
...
lval *builtin_lambda(lenv *e, lval *a) {
  // Check 2 arguments each of which is a qexpression
  LASSERT_NUM("\\", a, 2);
  ...
  for (int i = 0; i < a->cell[0]->count; i++) {
    LASSERT(a, a->cell[0]->cell[i]->type == LVAL_SYM,
            "Cannot define non-symbol expected %s got %s", ltype_name(LVAL_SYM),
            ltype_name(a->cell[0]->cell[i]->type));
  }

  // Pop first two arguments add them two lval_lambda
